...
public class TestUnit
{
	...
	try{ location.setYear((short)1200);  }
	catch(InvalidParameterException ipe)
	    { System.err.println("on peut pas ajouter 1200\n"+ipe.getMessage()); }
	System.out.println("subpath apres modif date: "+location.getSubPath());
	try{ location.setField("blabla"); }
	catch(InvalidParameterException ipe)
	    { System.err.println("on peut pas ajouter blabla\n"+ipe.getMessage()); }
	try{ location.setField(""); }
	catch(InvalidParameterException ipe)
	    { System.err.println("on peut pas ajouter \"\"\n"+ipe.getMessage()); }
	System.out.println("subpath apres modif matiere: "+location.getSubPath());
	
	
	System.out.println("\n<<<<Testing Exercise>>>>");
	Exercise e = new Exercise("Correction", "text", "title",
				  "description", "file name",
				  keyword_list, location);
	try{ e.setCorrection("Correction2"); }
	catch(InvalidParameterException ipe)
	    {System.err.println("on peut pas ajouter Correction2\n"+ipe.getMessage());}
	...
	System.out.println("\n<<<<Testing Project>>>>");
	Project p = new Project("text", "title",
				"description", "file name",
				keyword_list, location);
	...
	list_elements.add(e);
	TDUnit t = new TDUnit("title", "description", "file name",
			      keyword_list, location, list_elements);
	...
	System.out.println("getTeachingLocation apres modif: "+t.getTeachingLocation());
	Iterator it = t.getElementIterator(0);
	while(it.hasNext())
	    System.out.println("getTeachingLocation de it.next():"+(((Exercise)it.next()).getTeachingLocation()));
	System.out.println("Premier exercice: "+t.getSingleUnit(0).getTeachingLocation());
	...
	System.out.println("\n<<<<Testing Unit>>>>");
	Unit u = new Exercise("Correction4", "text4", "title4",
			      "description4", "file name4",
			      keyword_list, location);
	System.out.println("getTeachingLocation: "+u.getTeachingLocation());
